...
import DeleteCardFromFav from "./DeleteCardFromFav";
...
const FavoritesCards: FunctionComponent<FavoritesCardsProps> = () => {
    ...
    let getMyCards = async () => {
        try {
            let cardsIds = UserCtx.userctx.myCards;
            console.log(cardsIds);

            let cardsRes = await axios.get(
                `${process.env.REACT_APP_API}/cards?cardId=${cardsIds}`
            );
            let myCards: any = cardsRes.data;
            let userCards = myCards.filter((item: any) =>
    ...
    return (
        <>
            <div className="container mx-auto">
                ...
                <DeleteCardFromFav refresh={refresh} show={openDeleteModal} onHide={() => setOpenDeleteModal(!openDeleteModal)} cardId={cardId} />
