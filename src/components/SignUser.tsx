...
import { addUser, createMyCards } from "../services/userServices";
...
const SignUser: FunctionComponent<SignUserProps> = ({ isBusiness }) => {
    ...
    let formik = useFormik({
        ...
        onSubmit: (values: User) => {
            const newUser = { ...values, isBusiness: isBusiness, isLoggedIn: true };
            addUser({ ...newUser, isBusiness })
                .then((res) => {
                    console.log(newUser);
                    console.log(res.data);
                    UserCtx.userctx = ({ ...res.data });
                    UserCtx.changeUser({ ...res.data });
                    console.log(UserCtx.userctx);
                    if (!isBusiness) myCardsArr(res.data.id);
                    navigate("/about");
    ...
    let myCardsArr = (userId: number) => {
        createMyCards(userId).catch((err) => console.log(err))
    }
    return (<>
